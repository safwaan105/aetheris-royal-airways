...
export async function GET(request: Request) {
  ...
  if (oauthError) {
    return NextResponse.redirect(
      new URL(`/auth?oauth=google_${oauthError}`, baseUrl)
    );
  }

  // ✅ Proper env validation (fixes TypeScript issue)
  if (!clientId || !clientSecret) {
    return NextResponse.redirect(
      new URL("/auth?oauth=google_not_configured", baseUrl)
    );
  }
  ...
  if (!code || !state) {
    return NextResponse.redirect(
      new URL("/auth?oauth=google_failed", baseUrl)
    );
  }
  ...
  if (!expectedState || expectedState !== state) {
    return NextResponse.redirect(
      new URL("/auth?oauth=google_state_error", baseUrl)
    );
  }
  ...
  try {
    const tokenResponse = await fetch(
      "https://oauth2.googleapis.com/token",
      {
        ...
        body: new URLSearchParams({
          code,
          client_id: String(clientId),       // ✅ force string
          client_secret: String(clientSecret), // ✅ force string
    ...
    if (!tokenResponse.ok) {
      ...
      if (body.includes("redirect_uri_mismatch")) {
        return NextResponse.redirect(
          new URL("/auth?oauth=google_redirect_uri_mismatch", baseUrl)
        );
      }

      if (body.includes("invalid_client")) {
        return NextResponse.redirect(
          new URL("/auth?oauth=google_invalid_client", baseUrl)
        );
      }
    ...
    const identity = await parseGoogleIdToken(
      tokenData.id_token,
      clientId
    );
